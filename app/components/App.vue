    ...
    methods: {
        ...
        mergeAnswers(userAnswers) {
            ...
            this.currentQuestion.content.forEach((snippet, index) => {
                ...
                } else if(snippet.task_calculate) {
                    const task = snippet.task_calculate;
                    task.answers.forEach(
                    (answer, answerIndex) => {
                        answer.user_checked = answerIndex === userAnswer;
                        correctTasks +=
                            answer.user_checked && answer.correct ? 1*task.compute : 0;
                    });
                    totalTasks ++;
                    this.currentQuestion.task_calculate=true;
                } else if(snippet.results) { 
                    this.currentQuestion.results=snippet.results;
                } 
            });
